...
if __name__ == '__main__':
	...
	folders = ['Boudin', 'Caillebotte', 'Cezanne', 'Chase', 'Gauguin', 'Gorbatov', 'Guillaumin', 'Hassam', 'Korovin', 'Loiseau', 'Martin', 'Maufra', 'Metcalf', 'Monet', 'Pissarro', 'Renoir', 'Robinson', 'Rose', 'Sisley', 'Thaulow', 'Twachtman', 'Van Gogh', 'Weir']
	print(folders)

	for folder in folders:
		print(folder)
		images = os.listdir(PATH+'/'+folder+'/Originals')
		for image in images:
			print(PATH+'/'+folder+'/Originals/'+image)
			tenInput = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(PATH+'/'+folder+'/Originals/'+image))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
			tenOutput = estimate(tenInput)
			PIL.Image.fromarray((tenOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(PATH+'/'+folder+'/HED/'+image[:-4]+' HED.jpg')
# end
